...
environment or from `gdalraster::set_config_option()`. **No credential
ever passes through R** – only the non-secret knobs (endpoints, no-sign
flags) are translated R-side; secrets go straight from the environment
into the reader. With no static credentials the usual chain
(web-identity, ECS, EC2 instance metadata) still applies.
