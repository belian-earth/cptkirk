...
test_that("argument types are validated", {
  ...
  expect_error(warp_remote(f, file.path(dir, "o.tif"), te = c(1, 2, 3)),
               "xmin")
})
...
test_that("non-overlapping extent and the memory guard error clearly", {
  ...
  expect_error(
    warp_remote(f, o, t_srs = "EPSG:4326", te = c(0, 0, 1, 1), tr = c(0.01, 0.01)),
    "overlap"
  )
  # a tiny byte budget trips the guard
  expect_error(warp_remote(f, o, max_bytes = 10), "materialise")
})
